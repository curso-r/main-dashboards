...
Os *sinais de alerta* disparados por um valor reativo são um aviso dizendo que todos os outputs que dependem dele precisam ser recalculados. O destino desses sinais são as **funções observaroras**. Elas guardam o código de cada output e são o **ponto final do fluxo de reatividade**. 

As **funções observaroras** mais comuns são as funções `render*()`.
