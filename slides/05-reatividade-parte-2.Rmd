...
Nesses casos, podemos utilizar as funções `observer()` e `oberverEvent()`. Elas funcionam de maneira similar às funções `reactive()` e `eventReactive()`, mas em vez de criar expressões reativas, elas são observers.
...
<br>
<br>
`r blue('Ao RStudio: 19-observers.R')` e `r blue('Ao RStudio: 19-observeEvent.R')`
<center>
